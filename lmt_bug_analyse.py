...
if __name__ == '__main__':
    ...
    last_week_bug_status_df = pandas.read_excel(input_file2) # 读取上一周BUG状态数据
    current_bug_status_df = get_bug_status(bug_ids) # 获取当前bug状态数据

    # 对比BUG编号，检查是否有新增，若有新增，将新增的BUG保存到一个DataFrame
    mask = current_bug_status_df['Bug编号'].astype(int).isin(last_week_bug_status_df['Bug编号'].astype(int))
    increase_bug_df = current_bug_status_df[~mask]

    # 对边BUG状态，将有变化的BUG保存到一个DataFrame
    current_bug_status_df.set_index('Bug编号', inplace=False)
    last_week_bug_status_df.set_index('Bug编号', inplace=False)
    ...
    with pandas.ExcelWriter(output_file) as writer:
        current_bug_status_df.to_excel(writer, sheet_name="当前所有问题", index=False)
        increase_bug_df.to_excel(writer, sheet_name="新增问题", index=False)